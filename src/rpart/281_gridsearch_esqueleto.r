...
for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
  for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
    for (vmin_bucket in c(5, 10, 20, 50, 100, 200, 400, 600, 800, 1000)) {
      for (vcp in c(-1, -0.5, -0.25, 0, 0.25, 0.5, 1)) {
        
        # Si el arbol es muy profundo y con hojas muy chicas
        # y splits muy permisivos, no lo hago
        if (vmax_depth > 10 & (vmin_bucket < 100 | vmin_split < 100)) next
        
        if (vmin_bucket > vmin_split) next
        
        rel_split_bucket <- vmin_split / vmin_bucket
        
        if (rel_split_bucket < 2 | rel_split_bucket > 5) next
        
        # Imprimo el horario y los parametros del loop en una sola lìnea
        print( paste( Sys.time(), vmax_depth, vmin_split, vmin_bucket, vcp ) )
