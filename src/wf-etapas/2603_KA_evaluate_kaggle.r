#!/usr/bin/env Rscript
cat( "ETAPA  2603_KA_evaluate_kaggle.r  INIT\n")
...
cat( "ETAPA  2603_KA_evaluate_kaggle.r  START\n")
action_inicializar() 
    ...
    for (icorte in cortes)
    {
      ...
      if( "competition" %in% names(envg$PARAM) & submitear)
      {
        ...
        if( substr(res, 1, 12) == "Successfully" ) {
          res <- system( paste0("~/install/list3 ", nom_submit), intern= TRUE )
          cat( "res= ", res, "\n" )
...
cat( "ETAPA  2603_KA_evaluate_kaggle.r  END\n")
